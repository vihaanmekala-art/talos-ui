...
type MonteRandomizeResponse = { paths?: number[][] }
...
export type SyncedHoverState = { source: "price"; index: number } | { source: "backtest"; index: number } | null
...
const SignalPill = ({ signal }: { signal: string }) => {
  ...
  return (
    <span
      className={`font-mono rounded-full border px-3 py-0.5 text-[10px] font-bold tracking-[0.1em] uppercase ${map[signal] ?? "border-zinc-700 bg-zinc-900 text-zinc-400"}`}
    >
      {signal}
    </span>
  )
...
export function TickerNameBadge({
  ...
  return <span className="truncate font-mono text-[10px] text-zinc-600 max-w-[120px]">{data.name}</span>
}
...
export function OverviewSection({
  ...
  if (!data || !analysisData || analysisData.error || quote.error || analysis.error) return null
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3">
        ...
        <div className="ml-auto flex flex-wrap gap-5">
          {([["RSI", formatNumber(analysisData.rsi)], ["MACD", formatNumber(analysisData.macd)], ["Sharpe", formatNumber(analysisData.sharpe)]] as [string, string][]).map(([label, value]) => (
      ...
      <div className="grid grid-cols-4 gap-2 lg:grid-cols-8">
        ...
        />
        <StatCard label="S&P CAGR" value={formatPercent(analysisData.spy_cagr)} color="text-zinc-500" />
...
export function PriceChartPanel({
  ...
  const selectedRange = chartData && priceChartSelection ? getSelectionMetrics(chartData, priceChartSelection) : null
  const mirroredPriceIdx =
    shouldSync && syncedHover?.source === "backtest" ? mapHoverIndex(syncedHover.index, backtestLength, chartData?.length ?? 0) : undefined
  ...
  async function handleSimulateBlackSwan() {
    ...
    setBlackSwanData(null)
    try {
      const data = await fetchJson<BlackSwanResponse>(`${apiBase}/stock/${encodeURIComponent(ticker)}/black-swan`)
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[9px] tracking-[0.16em] uppercase text-zinc-600">Price · {PERIOD_MAP[period]}d</span>
          {ticker && (
            <button
              type="button"
            ...
            >
              ...
              {showBlackSwanOverlay && blackSwanData?.projected_path && (
                <Line
                  ...
                  data={chartData.map((point, index) => ({ ...point, blackSwan: blackSwanData.projected_path[index] ?? null }))}
                />
              )}
            </AreaChart>
          </ResponsiveContainer>
...
export function MonteCarloPanel({
  ...
  const probability =
    typeof targetProbability.data?.probability === "number"
      ? targetProbability.data.probability
      : typeof simulation.data.probability === "number"
        ? simulation.data.probability
        : null
  const mlReturn = typeof simulation.data.ml_expected_price === "number" ? simulation.data.ml_expected_price : null
          ...
          <div className={`mb-3 grid gap-2 ${probability !== null ? "grid-cols-3" : "grid-cols-2"}`}>
            ...
            {probability !== null && (
              <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-2.5">
                ...
                <p className="font-mono text-sm font-medium mt-1 text-zinc-200">{formatPercent(probability * 100)}</p>
              </div>
            )}
          </div>
        )}
...
export function ScenarioPanels({
    ...
    return (
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
        {["emerald", "red"].map(color => (
          <div key={color} className={`rounded-xl border ${color === "emerald" ? "border-emerald-900/50 bg-emerald-950/15" : "border-red-900/50 bg-red-950/15"} p-4`}>
            <SkeletonBlock className="h-3 w-20" />
  ...
  if (!data || data.error || error || (!data.bull_case && !data.bear_case)) return null
...
export function BacktestPanel({
  ...
  if (backtest.error) {
    return (
      <div className="rounded-xl border border-amber-600/25 bg-amber-950/15 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-amber-200/80">{getErrorMessage(backtest.error, "Backtest data is unavailable.")}</p>
      </div>
    )
  }
  ...
  const backtestReturn = typeof backtest.data.total_return === "number" ? backtest.data.total_return : null
  const mirroredBacktestIdx =
    shouldSync && syncedHover?.source === "price" ? mapHoverIndex(syncedHover.index, priceLength, backtestChartData.length) : undefined
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          <p className="font-mono text-[9px] text-zinc-600 mt-0.5">vs {formatPercent(backtest.data.buy_hold_return)} buy &amp; hold</p>
        </div>
      </div>
      <div className="mb-4 grid grid-cols-2 gap-2 lg:grid-cols-4">
        <StatCard label="Sharpe" value={formatNumber(backtest.data.sharpe)} />
        <StatCard label="Max Drawdown" value={formatPercent(backtest.data.max_drawdown)} color="text-red-400" />
            ...
            >
              ...
              <span
                className={`shrink-0 font-mono text-[9px] font-bold ${
                  article.sentiment === "Bullish" ? "text-green-400" : article.sentiment === "Bearish" ? "text-red-400" : "text-zinc-600"
                }`}
              >
                {article.sentiment}
              </span>
